...
def main():
    ...
    while running:
        ...
        # Fusionner toutes les bombes
        bombes_visibles = {}
        bombes_visibles.update(joueur.bombes)
        bombes_visibles.update(bot.bombes)

        # Le bot voit toutes les bombes
        bot.update(grille, bombes_visibles, joueur)
        ...
        # GESTION DES BOMBES - Chaque entité gère ses propres bombes
        ...
        if bot.Mort_joueur(grille):
            affichage_victoire(screen)
            recommencer_fin()
            grille = Grille()
            joueur = Joueur(1, 1)
            bot = Bot(ROW - 2, COL - 2)
...
def affichage_victoire(screen):
    screen.fill((0, 0, 0))

    font = pygame.font.SysFont("Arial", 48)
    text = font.render("Tu as gagné", True, (255, 0, 0))
